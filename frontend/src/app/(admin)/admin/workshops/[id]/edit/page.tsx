...
export default function EditWorkshopPage() {
  ...
  useEffect(() => {
    const fetchWorkshop = async () => {
      try {
        const response = await workshopsApi.getById(params.id as string);
        const workshop = response.data.data || response.data;
        reset({
          ...
          hasCertificate: workshop.hasCertificate || false,
          isPublished: workshop.isPublished || false,
        });
        if (workshop.syllabus) {
          setSyllabus(typeof workshop.syllabus === 'string' ? JSON.parse(workshop.syllabus) : workshop.syllabus);
        }
      } catch (error) {
        dispatch(addNotification({ message: 'Failed to load workshop', type: 'error' }));
        router.push('/admin/workshops');
      } finally {
        setIsLoading(false);
      }
    };
    if (params.id) fetchWorkshop();
  ...
  const onSubmit = async (data: WorkshopFormData) => {
    ...
    try {
      await workshopsApi.update(params.id as string, { ...data, syllabus });
      dispatch(addNotification({ message: 'Workshop updated successfully!', type: 'success' }));
      router.push('/admin/workshops');
    } catch (error: any) {
      dispatch(addNotification({ message: error.response?.data?.message || 'Failed to update workshop.', type: 'error' }));
    } finally {
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <h1 className="text-2xl font-display font-bold text-gray-900 dark:text-nexus-text">Edit Workshop</h1>
      </div>
